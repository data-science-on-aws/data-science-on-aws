...
class AmazonReviewsProcessor(DataProcessor):
  """Processor for the Amazon Customer Reviews Dataset."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    examples = []
    print('**** set_type {}'.format(set_type))
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  print('**** EXAMPLE.GUID {}'.format(example.guid))
  print('**** EXAMPLE.TEXT_A {}'.format(example.text_a))
  print('**** EXAMPLE.TEXT_B {}'.format(example.text_b))
  print('**** EXAMPLE.LABEL {}'.format(example.label))
  if example.label != -1:
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    eval_drop_remainder = FLAGS.use_tpu
  ...
  if FLAGS.do_predict:
    ...
    print('*** RECEIVED PREDICT EXAMPLES ***')
    ...
    predict_drop_remainder = FLAGS.use_tpu
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        if i >= num_actual_predict_examples:
          break
        output_line = str(np.argmax(probabilities)) + '\t' + "\t".join(
