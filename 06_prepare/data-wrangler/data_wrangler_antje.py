...
def custom_formula(df, spark, formula, output_column=None):
    output_df = df.selectExpr(*df.columns, formula)
    if output_column:
        output_df = output_df.withColumnRenamed(output_df.columns[-1], output_column)
