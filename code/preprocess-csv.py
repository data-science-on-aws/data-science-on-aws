...
def main():
    ...
    from pyspark.sql.types import StringType, IntegerType, FloatType, TimestampType, ByteType, LongType
    ...
    df_joined = spark.read.csv(
        os.path.join("s3://", args.s3_input_bucket, args.s3_input_key_prefix),
        schema=schema, 
        header=True)
    df_joined.show()

#     df_ride_fare = spark.read.csv(
...
#     # Write out the data
    df_train.write.mode("overwrite").csv(os.path.join("s3://", args.s3_output_bucket, args.s3_output_key_prefix, "train"))
    df_validation.write.mode("overwrite").csv(os.path.join("s3://", args.s3_output_bucket, args.s3_output_key_prefix, "validation"))
